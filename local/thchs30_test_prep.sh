...
# This script prepares the test data directory for thchs30
...
echo "creating data/thchs30/test"
mkdir -p data/thchs30/test

#create wav.scp, utt2spk.scp, spk2utt.scp, text
(
for x in test; do
  echo "cleaning data/$x"
  cd $dir/data/thchs30/$x
  ...
  for nn in `find  $corpus_dir/$x -name "*.wav" | sort -u | xargs -I {} basename {} .wav`; do
      ...
      echo $uttid `sed -n 1p $corpus_dir/data/$nn.wav.trn` >> text
  done
...
utils/utt2spk_to_spk2utt.pl data/thchs30/test/utt2spk > data/thchs30/test/spk2utt
